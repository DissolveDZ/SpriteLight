...
{
    if (index_or_file_path < 0)
    {
        return NULL; // Invalid index or file path
    }
    Salad *salad = state->salad;
    HashNode *current = salad->hash_table->table[index_or_file_path % salad->hash_table->capacity];
    while (current)
    {
        if (current->resource.index == index_or_file_path || strcmp(current->resource.file_path, (const char *)index_or_file_path) == 0)
        {
            return &(current->resource);
        }
        current = current->next;
...
void RemoveResource(int index_or_file_path)
{
    ...
    Salad *salad = state->salad;
    int index = (int)index_or_file_path % (int)salad->hash_table->capacity;
    ...
    while (current)
    {
        if (current->resource.index == index_or_file_path || strcmp(current->resource.file_path, (const char *)index_or_file_path) == 0)
        {
            if (prev)
                ...
                salad->hash_table->table[index] = current->next;
            free(current);
            salad->hash_table->size--;
            return;
        }
