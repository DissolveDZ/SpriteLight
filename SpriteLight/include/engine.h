...
typedef enum CameraType { DEFAULT_CAMERA, PANNING_CAMERA } CameraType;
...
extern Shader downsample_shader, upsample_shader, general_shader, ui_shader,
    text_shader, gradient_shader, circle_shader, text_shader_world, text_shader,
    sdf_text_shader;
...
void GBufferSetup(unsigned int *g_buffer, unsigned int *g_position,
                  unsigned int *g_normal, unsigned int *g_albedo,
                  unsigned int *depth, int screen_width, int screen_height);
void PostProcessBuffer(unsigned int *post_process_fbo,
                       unsigned int *post_process_color, unsigned int *depth,
                       int screen_width, int screen_height);
void BufferSetup(unsigned int *VAO, unsigned int *VBO, float vertices[],
                 int size, bool textured, bool normals);
...
// initiate engine, higher mip level = more bloom samples and 0 is no bloom at
// all
State *EngineInit(char *window_name, char *icon_path, int width, int height,
                  int bloom_mip_level);
void EngineUpdate();
...
void RenderBloom(unsigned int src_texture, float filter_radius, float threshold,
                 float knee);

#endif // SPhttps://github.com/smoka7/multicursors.nvimRITELIGHT_H
